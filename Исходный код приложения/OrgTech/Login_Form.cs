...
namespace GUI
{
    public partial class Login_Form_Window : Form
    {
        ...
        public void btn_Login_Click(object sender, EventArgs e)
        {
            ...
            string query_string = $"select userID, fio, login, password, userStatusID, photo from Users where login = '{loginUser}' and password = '{passwordUser}'";
            ...
            if (table.Rows.Count == 1)
            {
                ...
                byte[] imageData = null;
                if (table.Rows[0].Table.Columns.Contains("photo") && !table.Rows[0].IsNull("photo"))
                {
                    imageData = (byte[])table.Rows[0]["photo"];
                }
                ...
                if (userStatusID == 4)
                {
                    Manager managerForm = new Manager(user_FIO, user_role, userID, userImage);
                    this.Hide();
                ...
                {
                    Master masterForm = new Master(user_FIO, user_role, userID, userImage);
                    this.Hide();
                ...
                {
                    OperatorForm operatorForm = new OperatorForm(user_FIO, user_role, userID, userImage);
                    this.Hide();
                ...
                {
                    Client clientForm = new Client(user_FIO, user_role, userID, userImage);
                    this.Hide();
        ...
        private void viewPassBox_CheckedChanged(object sender, EventArgs e)
        {
            if (viewPassBox.Text == "показать пароль")
            {
                textBox_PasswordField.UseSystemPasswordChar = false;
                viewPassBox.Text = "скрыть пароль";
            }
            else
            {
                textBox_PasswordField.UseSystemPasswordChar = true;
                viewPassBox.Text = "показать пароль";
            }
        }
    }
}
