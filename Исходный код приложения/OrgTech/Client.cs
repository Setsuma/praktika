...
namespace GUI
{
    public partial class Client : Form
    {
        ...
        public Client(string fullName, string role, int id, Image photoImage)
        {
            InitializeComponent();
            userNF = fullName;
            userR = role;
        ...
        private void ClientForm_Load(object sender, EventArgs e)
        {
            ...
            pictureBox_base_photo.Image = photo;
            LoadR(userID);
            UpdateCount(userID);
        }
            ...
            var columns = new Dictionary<string, string>
{
    ...
    { "typeName", "Тип устройства" }
        ...
        {
            string googleFormsUrl = "https://docs.google.com/forms/d/e/1FAIpQLSeGTXQ_W2oJkFtAuZ9Mfg492lw-ae9-ONUWVWHDW3dkzvUHiA/viewform?usp=sf_link";
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                pictureBox_base_photo.Image = Image.FromFile(openFileDialog.FileName);
            }
            if (pictureBox_base_photo.Image != null)
            {
                string savePhotoQuery = @"
            UPDATE Users
            SET photo =  @ImageData
            WHERE userID = @UserID";
                using (SqlCommand command = new SqlCommand(savePhotoQuery, database.GetConnection()))
                {
                    try
                    {
                        database.openConnection();
                        Image image = pictureBox_base_photo.Image;
